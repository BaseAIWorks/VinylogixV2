...
export async function createOrderRequest(user: User, cartItems: CartItem[], distributorId: string): Promise<Order> {
    if (cartItems.length === 0) {
        throw new Error("Cannot create an order with an empty cart.");
    }
    ...
    const shippingAddress = [
      ...
      `${user.postcode || ''} ${user.city || ''}`.trim(), user.country
    ].filter(Boolean).join('\n');
    ...
    if (user.useDifferentBillingAddress && user.billingAddress) newOrderData.billingAddress = user.billingAddress;
    if (user.phoneNumber) newOrderData.phoneNumber = user.phoneNumber;
    if (user.companyName) newOrderData.customerCompanyName = user.companyName;
    if (user.vatNumber) newOrderData.customerVatNumber = user.vatNumber;
    if (user.eoriNumber) newOrderData.customerEoriNumber = user.eoriNumber;
    if (user.chamberOfCommerce) newOrderData.customerChamberOfCommerce = user.chamberOfCommerce;
    ...
    await addDoc(notificationsCollectionRef, {
        distributorId,
        type: 'new_order' as const,
        message: `New order request from ${user.email || 'a client'} awaiting your approval.`,
        orderId: orderDocRef.id,
        orderTotal: totalAmount,
        customerEmail: user.email || 'N/A',
        createdAt: Timestamp.fromDate(now),
